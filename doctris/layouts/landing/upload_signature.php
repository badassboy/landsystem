...
if (isset($id)) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $alloc_number = $row['allocation_number'];
    ...
    if(isset($_POST['upload'])){
        ...
        $uploaded = $api->uploadSignature($file_name,$id);
        if ($uploaded) {
             echo "<script>alert('site plan uploaded')</script>";
             // demo purposes.delete the spcific land after signature is uploaded.
             $stmt = $db->prepare("DELETE FROM land WHERE id =?");
             $stmt->execute([$_GET['id']]);
             $deleted = $stmt->rowCount();
             if ($deleted>0) {
                 
                 // insert the deleted record into the database.
                $stmt =$db->prepare("INSERT INTO trash(id,firstName,lastName,allocation_number,plot_size)
                    VALUES(?,?,?,?,?)");
                $stmt->execute([$land_id,$firstName,$lastName,$alloc_number,$plot_size]);
                $data = $stmt->rowCount();
                if ($data>0) {
                    // code...
                }else{
                    ...
                    <div class="col-xl-9 col-lg-8 col-md-7 mt-4 pt-2 mt-sm-0 pt-sm-0">
                        <div class="row">
                            <div class="col-xl-9 col-lg-6 col-md-4">
                                <h5 class="mb-0">Upload Signature</h5>
                            </div><!--end col-->
